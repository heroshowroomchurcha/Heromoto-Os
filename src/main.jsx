...
function App() {
  ...
  return <div className="app-shell"><aside className="sidebar"><div className="brand"><div className="brand-mark">R</div><div><strong>rideflow</strong><span>SHOWROOM OS</span></div></div><div className="branch-select"><span className="online-dot"/> Hero MotoCorp <b>·</b> Andheri <Icon name="arrow" size={13}/></div><nav>{visibleNav.map(item => <button key={item.label} className={active === item.label ? 'nav-item active' : 'nav-item'} onClick={() => setActive(item.label)}><Icon name={item.icon}/><span>{item.label}</span>{item.count && <em>{item.count}</em>}</button>)}</nav><div className="sidebar-bottom"><div className="sync"><span className="pulse"/><div><b>Supabase synced</b><small>{isOwner ? 'Owner access · Secure' : 'Staff access · Limited'}</small></div></div><button className="user-row" onClick={() => supabase.auth.signOut()}><div className="avatar me">{(profile?.full_name || session.user.email || 'NK').slice(0,2).toUpperCase()}</div><div><b>{profile?.full_name || session.user.email}</b><small>{isOwner ? 'Showroom owner' : 'Showroom staff'} · Sign out</small></div><Icon name="dots" size={16}/></button></div></aside>
    <main className="main-content"><header className="topbar"><div className="breadcrumb"><div className="topbar-brand"><img src="/hero.png" alt="Hero MotoCorp" style={{height:'56px', width:'auto', objectFit:'contain'}}/></div></div><div className="top-actions"><label className="search"><Icon name="search" size={17}/><input value={query} onChange={e => setQuery(e.target.value)} placeholder="Search vehicles, customers..."/></label><button className="icon-button" onClick={() => { setToast('No new notifications'); setTimeout(() => setToast(''), 3000); }}><Icon name="bell"/></button><button className="primary-button" onClick={() => setActive('Inventory')}><Icon name="plus" size={17}/> New sale</button></div></header><div className="content-wrap"><section className="page-heading"><div><p className="eyebrow">{dashboardDateLabel(clock)} <span className="live"><i/> LIVE</span></p><h1>{active === 'Overview' ? `${greetingForHour(clock.getHours())}, Nihal.` : active}</h1><p className="subcopy">{active === 'Overview' ? 'Your showroom numbers, inventory and follow-ups in one view.' : active === 'Inventory' ? 'Show customers every available Hero bike, price and offer limit.' : active === 'Other bikes' ? 'Show customers other brand bikes (Honda, TVS, etc.) in stock.' : active === 'Second hand' ? 'Manage pre-owned bikes, buyback from customers and sell them.' : `Manage your ${active.toLowerCase()} records from one place.`}</p></div><button className="date-button"><Icon name="calendar" size={16}/> Supabase live <span className="live-dot"/></button></section>
      ...
      : active === 'Sales' ? <SalesWorkspace sales={filteredSales} customers={filteredVisibleCustomers} userId={session.user.id} onChange={setSales} onToast={setToast} />
...
function Overview({ sales, inventory, secondHandInventory, monthlySales, stockTotal, rcRecords, setActive, onDownloadBackup }) {
...
function SalesWorkspace({ sales, customers, userId, onChange, onToast }) {
...
function InvoiceModal({ record, onClose }) {
  ...
  return <div className="modal-backdrop invoice-backdrop" onClick={onClose}><div className="invoice-sheet" onClick={e => e.stopPropagation()}><div className="invoice-actions"><button className="ghost-button" onClick={onClose}>Close</button><button className="ghost-button" disabled={pdfBusy} onClick={shareWhatsApp}>Send on WhatsApp</button><button className="primary-button" disabled={pdfBusy} onClick={downloadPdf}>{pdfBusy ? 'Preparing PDF…' : 'Download PDF'}</button></div><div className="invoice-paper" ref={invoiceRef}><div className="invoice-top"><div><div className="invoice-brand" style={{display: 'flex', alignItems: 'center'}}>{logoUrl ? <img src={logoUrl} alt={waBrand} style={{height:'48px', width:'auto', objectFit:'contain'}}/> : <div className="brand-mark" style={{width:'36px', height:'36px', background:'#212326', color:'#fff', borderRadius:'6px', display:'grid', placeItems:'center', marginRight:'12px', fontWeight:'bold', fontSize:'18px'}}>R</div>}<div><b>rideflow</b><small>SHOWROOM OS</small></div></div><p>{showroomSubText}<br/>Mumbai, Maharashtra · +91 22 4000 2026</p></div><div className="invoice-meta"><small>TAX INVOICE</small><strong>{invoiceNo}</strong><span>{dateLabel(sale.sale_date)} · {new Date(sale.sale_date).toLocaleTimeString('en-IN', { hour: '2-digit', minute: '2-digit' })}</span></div></div><div className="invoice-rule"/><div className="invoice-parties"><div><small>BILLED TO</small><b>{customer.name}</b><span>{customer.phone || 'Phone not added'}</span><span>Aadhaar · •••• {customer.aadhaar_last4 || 'Not added'}</span><span>PAN · {customer.pan_masked || 'Not added'}</span></div><div><small>VEHICLE DETAILS</small><b>{vehicle.model}</b><span>{vehicle.variant} · {vehicle.cc || '—'} cc</span><span>Colour · {vehicle.color}</span></div></div><div className="invoice-items"><div className="invoice-item invoice-item-head"><span>DESCRIPTION</span><span>AMOUNT</span></div><div className="invoice-item"><span><b>{vehicle.model} {vehicle.variant}</b><small>On-road vehicle price</small></span><strong>{money(vehicle.on_road_price || vehicle.ex_showroom_price)}</strong></div><div className="invoice-item discount-row"><span>Showroom discount</span><strong>− {money(discount)}</strong></div></div><div className="invoice-total"><span>Total payable</span><strong>{money(saleAmount)}</strong></div><div className="invoice-footer"><div><b>{thankYouText}</b><span>{rcText}</span></div><div>{signature && <div className="invoice-signature"><img src={signature} alt="Customer digital signature"/><small>Digitally signed by customer</small></div>}<small>PAYMENT STATUS</small><strong>{sale.payment_status || 'Pending'}</strong></div></div></div></div></div>;
}
