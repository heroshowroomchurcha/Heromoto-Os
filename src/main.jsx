...
function SalesWorkspace({ sales, userId, onChange, onToast }) {
  ...
  async function clearPendingPayment(sale) {
    const confirmation = window.confirm(`Is ${sale.customer_name} ka payment clear ho gaya hai? Is action ko undo nahi kiya ja sakta.`);
    if (!confirmation) return;
    const { data, error } = await supabase.from('sales').update({ payment_status: 'Paid', updated_at: new Date().toISOString() }).eq('id', sale.id).select().single();
