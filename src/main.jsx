...
function Inventory({ vehicles, onSell, onDetails }) {
...
function SalesWorkspace({ sales, userId, onChange, onToast }) {
  ...
  return <section className="sales-workspace"><div className="sales-workspace-head"><div><p className="eyebrow">CONNECTED SALES</p><h2>All sales</h2><p>Edit returns, exchanges, discounts and RC records from here.</p></div><span className="inventory-count">{sales.length} records</span></div>{sales.length ? <div className="panel sales-records"><div className="table-head sales-record-head"><span>Customer</span><span>Vehicle</span><span>Sale value</span><span>Date</span><span>Payment</span><span></span></div>{sales.map((sale, index) => <div className="sales-record-row" key={sale.id || index}><div className="customer"><div className="avatar" style={{background:['#e8d4ce','#dce3d3','#d9dcea','#e8e0cc'][index % 4]}}>{String(sale.customer_name || 'NK').split(' ').map(x => x[0]).join('').slice(0,2)}</div><div><b>{sale.customer_name}</b><small>{sale.customer_phone || 'No phone added'}</small></div></div><span className="vehicle-highlight"><Icon name="bike" size={14}/><b>{sale.vehicle_name || 'Vehicle not added'}</b></span><strong>{money(sale.sale_amount)}</strong><span className="muted">{dateLabel(sale.sale_date)}</span><span className={sale.sale_type === 'Return' ? 'status danger' : sale.sale_type === 'Exchange' ? 'status info' : sale.payment_status === 'Paid' ? 'status success' : 'status warning'}><i/>{sale.sale_type || sale.payment_status || 'Pending'}</span><div className="sale-row-actions"><button className="text-button" onClick={() => setSelectedSale(sale)}>View</button><button className="text-button" onClick={() => setEditingSale(sale)}>Edit</button></div></div>)}</div> : <div className="panel placeholder-panel"><div className="empty-icon"><Icon name="chart" size={27}/></div><h2>No sales yet</h2><p>Inventory se sale save karte hi yahan sync ho jayegi.</p></div>}{selectedSale && <CustomerDetailModal sale={selectedSale} onClose={() => setSelectedSale(null)} />}{editingSale && <SaleEditModal sale={editingSale} onClose={() => setEditingSale(null)} onSave={saveSaleUpdate} />}</section>;
}
...
function FinanceWorkspace({ sales }) {
  const financeSales = sales.filter(sale => sale.payment_status === 'Finance');
  const totalValue = financeSales.reduce((sum, sale) => sum + Number(sale.sale_amount || 0), 0);
  return <section className="finance-workspace"><div className="finance-workspace-head"><div><p className="eyebrow">FINANCE SALES</p><h2>Finance desk</h2><p>All financed vehicles stored separately from regular sales.</p></div><div className="finance-summary"><b>{financeSales.length}</b><span>financed sales</span><b>{money(totalValue)}</b><span>total value</span></div></div>{financeSales.length ? <div className="panel finance-records"><div className="table-head finance-record-head"><span>Customer</span><span>Vehicle</span><span>Amount</span><span>Sale date</span><span>Status</span></div>{financeSales.map((sale, index) => <div className="finance-record-row" key={sale.id || index}><div className="customer"><div className="avatar" style={{background:['#e8d4ce','#dce3d3','#d9dcea','#e8e0cc'][index % 4]}}>{String(sale.customer_name || 'NK').split(' ').map(x => x[0]).join('').slice(0,2)}</div><div><b>{sale.customer_name}</b><small>{sale.customer_phone || 'No phone added'}</small></div></div><span className="muted">{sale.vehicle_name}</span><strong>{money(sale.sale_amount)}</strong><span className="muted">{dateLabel(sale.sale_date)}</span><span className="status info"><i/>Finance</span></div>)}</div> : <div className="panel placeholder-panel"><div className="empty-icon"><Icon name="chart" size={27}/></div><h2>No finance sales yet</h2><p>Sale modal me Payment status ko Finance select karoge to record yahan aa jayega.</p></div>}</section>;
}
...
function SignaturePad({ value, onChange }) {
  ...
  const move = event => { if (!drawing.current) return; const canvas = canvasRef.current; const ctx = canvas.getContext('2d'); const p = point(event); ctx.lineWidth = 2; ctx.lineCap = 'round'; ctx.strokeStyle = '#252524'; ctx.lineTo(p.x, p.y); ctx.stroke(); onChange(canvas.toDataURL('image/png')); };
  const end = () => { drawing.current = false; };
  const drawTypedName = name => { const canvas = canvasRef.current; const ctx = canvas.getContext('2d'); ctx.clearRect(0, 0, canvas.width, canvas.height); if (name.trim()) { ctx.fillStyle = '#252524'; ctx.font = '600 46px Caveat, cursive'; ctx.textBaseline = 'middle'; ctx.fillText(name.trim(), 24, canvas.height / 2); onChange(canvas.toDataURL('image/png')); } else onChange(''); };
...
function InvoiceModal({ record, onClose }) {
  ...
  async function shareWhatsApp() {
    ...
    if (navigator.share && (!navigator.canShare || navigator.canShare({ files: [file] }))) {
      await navigator.share({ files: [file], title: invoiceNo, text: `Hello ${customer.name}, your Hero MotoCorp invoice ${invoiceNo} is attached.` });
      return;
    }
    const phone = String(customer.phone || '').replace(/\D/g, '');
    const text = encodeURIComponent(`Hello ${customer.name}, your Hero MotoCorp invoice ${invoiceNo} is ready. Total: ${money(saleAmount)}. Please attach the downloaded PDF in this chat.`);
    window.open(`https://wa.me/${phone}?text=${text}`, '_blank', 'noopener,noreferrer');
    const link = document.createElement('a'); link.href = URL.createObjectURL(blob); link.download = `${invoiceNo}.pdf`; link.click(); URL.revokeObjectURL(link.href);
  }
